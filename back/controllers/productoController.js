...
const listar_productos_admin = async function (req, res) {
  if (req.user) {
    if (req.user.role == "admin") {
      ...
      var reg = await Producto.find({ titulo: new RegExp(filtro, "i") });
...
const actualizar_variedades_admin = async function (req, res) {
  if (req.user) {
    if (req.user.role == "admin") {
      let id = req.params["id"];
      let data = req.body;
...
const listar_productos_publico = async function (req, res) {
  var filtro = req.params["filtro"];
  var reg = await Producto.find({ titulo: new RegExp(filtro, "i") }).sort({
    createdAt: -1,
  });
  res.status(200).send({ data: reg });
};
...
const listar_productos_recomendados_publico = async function (req, res) {
  ...
  var reg = await Producto.find({ categoria: categoria })
    .sort({
      createdAt: -1,
    })
    .limit(8);
